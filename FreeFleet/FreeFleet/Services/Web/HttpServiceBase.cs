        ...
        public async Task<EventFleet[]> GetEventFleetsAsync(string host)
        {
            ...
            try
            {
                ...
                fleets.AddRange(from node in eventFleetNodes
                    let coordOrigin = node.SelectSingleNode("./td[@class='coordsOrigin']/a")?.InnerHtml.Trim() ??
                                      SharedResources.NodeNotFound
                    let coordDest = node.SelectSingleNode("./td[@class='destCoords']/a")?.InnerHtml.Trim() ??
                                    SharedResources.NodeNotFound
